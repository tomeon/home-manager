...
let
  ...
  # *without* co-opting a legal value (like `null`) for signifying omission.
  ...
  mkExampleFromSubOptions =
    subOptions:
    let
      filtered = lib.filterAttrs (name: _: !(lib.hasPrefix "_" name)) subOptions;
    in
    lib.mapAttrs (name: value: value.example) filtered;

  mkExampleFromSubmodule = type: mkExampleFromSubOptions (type.getSubOptions { });

  commandType = types.nullOr (
    types.coercedTo (types.nonEmptyListOf types.str) lib.escapeShellArgs types.nonEmptyStr
  );

  filterType = types.nullOr (
    types.coercedTo (types.listOf types.str) (lib.concatStringsSep ",") (types.separatedString ",")
  );

  conditionType = types.submodule (
    { config, options, ... }:
    {
      ...
      options = {
        ...
        priority = mkOmittableOption {
          type =
            ...
            in
            types.coercedTo intType (lib.elemAt strings) stringType;
          description = ''
...
in
{
  ...
  options.programs.ntfy-sh = {
          ...
          options =
            ...
            in
            {
              ...
              subscribe = mkOption {
                ...
                example =
                  let
                    subOptions = options.subscribe.type.nestedTypes.elemType.getSubOptions { };
                  in
                  {
                    just-log-json = {
                      topic = subOptions.topic.example;
                    };
    ...
    config = mkOption {
      ...
      default =
        let
          serializable = lib.filterAttrsRecursive (_: value: !(unserializableType.check value)) cfg.settings;
        in
        serializable
        // {
          subscribe = map (
            ...
            in
            (builtins.removeAttrs subscription [ "condition" ])
    ...
    shellWrapper = mkOption {
      ...
      default = pkgs.runCommand "ntfy-shell-wrapper" { } ''
        mkdir -p "$out/bin"
        ${pkgs.coreutils}/bin/ln -s ${cfg.shell}/${lib.escapeShellArg cfg.shell.shellPath} "$out/bin/sh"
      '';
    };
    ...
    writeConfig = mkOption {
      ...
      default = pkgs.writeShellScript "write-ntfy-sh-config" ''
        ...
        ${lib.optionalString cfg.enableSecretsReplacement (
          let
            tmp = ".${cfg.configFileName}.tmp";
          in
          ''
            src=${lib.escapeShellArg tmp}
            ${utils.genJqSecretsReplacementSnippet cfg.config tmp}
            install_config() {
              ${pkgs.coreutils}/bin/mv -f "$1" "$2"
            }
          ''
        )}

        ${lib.optionalString (!cfg.enableSecretsReplacement) ''
          src=${lib.escapeShellArg cfg.configFile}
          install_config() {
            ${pkgs.coreutils}/bin/install -Dm0640 "$1" "$2"
          }
        ''}

        install_config "$src" ${lib.escapeShellArg cfg.configFileName}
      '';
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    home.activation.ntfy-sh =
      lib.hm.dag.entryAfter
        [
          "reloadSystemd" # secrets may depend on (e.g.) the `agenix` oneshot
          "writeBoundary"
        ]
        ''
          $DRY_RUN_CMD ${cfg.writeConfig} "''${XDG_CONFIG_HOME:-"''${HOME}/.config"}/ntfy"
        '';
  };
}
