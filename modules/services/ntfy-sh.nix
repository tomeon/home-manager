  ...
  options.services.ntfy-sh = {
    ...
    script = mkOption {
      ...
      default = pkgs.writeShellScript "ntfy-sh-start" ''
        ...
        # `RUNTIME_DIRECTORY` is a colon-separated list of directories; strip
        # all but the first element.
        config="''${runtime_directory%%:*}"/${lib.escapeShellArg prog.configFileName}
  ...
  config = lib.mkIf cfg.enable {
    systemd.user.services.ntfy-sh = {
      Unit = {
        ...
        X-Restart-Triggers = [ (builtins.hashString "md5" (builtins.toJSON prog.config)) ];
      };
