...
in
{
  ...
  nmt.script =
    ...
    ''
      ...
      assertLinkPointsTo "$shellWrapper/bin/sh" "$shell"/${lib.escapeShellArg cfg.shell.shellPath}
      assertFileRegex "$wrapper/bin/ntfy" "PATH=$shellWrapper/bin"

      echo ${lib.escapeShellArg expected.default-password} > ${lib.escapeShellArg (builtins.baseNameOf cfg.settings.default-password._secret)}
